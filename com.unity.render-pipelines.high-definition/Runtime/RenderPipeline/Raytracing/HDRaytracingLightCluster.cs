...
namespace UnityEngine.Rendering.HighDefinition
{
    ...
    class HDRaytracingLightCluster
    {
        ...
        void BuildLightData(CommandBuffer cmd, HDCamera hdCamera, HDRayTracingLights rayTracingLights, DebugDisplaySettings debugDisplaySettings)
        {
            ...
            HDLightEntityCollection lightEntities = HDLightEntityCollection.instance;
            HDRenderPipeline.ProcessedLightParameters processLightParameter = new HDRenderPipeline.ProcessedLightParameters();
            processLightParameter.shadowMapFlags = HDVisibleLightEntities.ShadowMapFlags.None;

            for (int lightIdx = 0; lightIdx < lightEntities.lightCount; ++lightIdx)
            {
                // Grab the additinal light data to process
                HDAdditionalLightData additionalLightData = lightEntities.hdAdditionalLightData[lightIdx];
                additionalLightData = lightEntities.hdAdditionalLightData[lightIdx];
                ...
                processLightParameter.gpuLightType = gpuLightType;
                processLightParameter.lightType = additionalLightData.type;
                processLightParameter.distanceToCamera = (lightEntities.lightTransforms[lightIdx].position - hdCamera.camera.transform.position).magnitude;
                processLightParameter.lightDistanceFade = HDUtils.ComputeLinearDistanceFade(processLightParameter.distanceToCamera, lightEntities.fadeDistances[lightIdx]);
                processLightParameter.lightVolumetricDistanceFade = HDUtils.ComputeLinearDistanceFade(processLightParameter.distanceToCamera, lightEntities.volumetricFadeDistances[lightIdx]);
                processLightParameter.isBakedShadowMask = HDRenderPipeline.IsBakedShadowMaskLight(lightComponent);
                ...
                m_RenderPipeline.GetLightData(
                    cmd, hdCamera, hdShadowSettings, visibleLight, lightComponent,
                    lightEntities, processLightParameter,
                    shadowIndex, lightIdx, additionalLightData, contactShadowScalableSetting, isRasterization: false, ref lightDimensions, ref screenSpaceShadowIndex, ref screenSpaceChannelSlot, ref lightData);
